# Правила внесения вклада (Contribution Rules) для MinePrime
...
- Все коммиты должны быть подписаны и должны отображаться в GitHub как `Verified`.
- Pull request должны быть сфокусированными и доступными для технического обзора.
- Не смешивайте несвязанные изменения в одном pull request.
- Не отправляйте сгенерированные артефакты, секреты, токены, приватные ключи, хранилища ключей или учетные данные для продакшена.
- Не отправляйте код, скопированный из сторонних источников, если только Оператор не одобрил источник и лицензию в письменном виде.

## Принятие вклада

Оператор может принимать, отклонять, запрашивать изменения, откладывать или игнорировать любой вклад по своему усмотрению.

Объединенные (merged) или иным образом принятые вклады обрабатываются в соответствии с `DEVELOPER_AGREEMENT.md`.

## Контакты

Вопросы по юридическим условиям, статусу вклада и доступам можно направлять на `admin@mineprime.ru`.

---

# Contribution Rules for MinePrime
...
- All commits must be signed and should display `Verified` on GitHub.
- Pull requests should be focused and technically reviewable.
- Do not mix unrelated changes in one pull request.
- Do not submit generated artifacts, secrets, tokens, private keys, keystores, or production credentials.
- Do not submit code copied from third-party sources unless the Operator has approved the source and license in writing.

## Acceptance of contributions

The Operator may accept, reject, request changes to, postpone, or ignore any contribution at their sole discretion.

Merged or otherwise accepted contributions are handled under `DEVELOPER_AGREEMENT.md`.

## Contact

Questions about legal terms, contribution status, and access can be sent to `admin@mineprime.ru`.
